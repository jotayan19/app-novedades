...
<style>
    ...
    .login-title { font-size: 1.8em; font-weight: bold; color: #4CAF50 !important; margin-bottom: 10px; text-transform: uppercase; letter-spacing: 1px; }
...
def obtener_horario_aula(aula):
    ...
    for hor in horarios:
        if len(hor) >= 2 and hor[0].strip().upper() == aula.strip().upper():
            return hor[1]
    return "" # Cambiado a vacío para que no asuma 06:20 por defecto si no está configurado
...
def calcular_estadisticas():
    ...
    for emp in empleados:
        if len(emp) >= 5:
            ...
            if tiene_novedad: aulas_dict[aula]["ausentes"] += 1
    ...
    # Calcular ingreso diferenciado (todos los que NO son 06:20)
    total_dif = sum(info["total"] for info in aulas_dict.values() if info["horario"] != "06:20")
    ausentes_dif = sum(info["ausentes"] for info in aulas_dict.values() if info["horario"] != "06:20")
    ...
    return {
        ...
        "ingreso_diferenciado": presentes_dif # Nuevo dato para la minuta
    }
...
# ==================== TAB 2: AULAS Y HORARIOS (MODIFICADO) ====================
with tab2:
    ...
    with st.form("form_horarios"):
        ...
        for i, aula in enumerate(AULAS):
            with cols[i % 2]:
                horario_actual = horarios_dict.get(aula, "") # Ahora empieza vacío si no está guardado
                opciones = ["", "06:20", "07:30", "08:10", "09:20"]
                index_val = 0
                ...
                if horario: nuevos_horarios.append([aula, horario]) # Solo guarda si seleccionó algo
    ...
    if stats:
        ...
        with col2:
            ...
            st.metric("Total", sum(info["total"] for info in stats["aulas_dict"].values() if info["horario"] not in ["06:20", ""]))
            st.metric("Presentes", sum(info["total"]-info["ausentes"] for info in stats["aulas_dict"].values() if info["horario"] not in ["06:20", ""]))
...
with tab4:
    ...
    if stats:
        ...
        with col1: st.metric("️ Ausentes Total", stats["ausentes_total"])
    ...
    else: st.warning("Primero importa el personal")

# ==================== TAB 5: RACIONAMIENTO (MODIFICADO: SOLO ALMUERZO) ====================
with tab5:
    ...
    if empleados:
        ...
        with col2:
            ...
            if st.button("Guardar Almuerzo", type="primary", use_container_width=True):
                if empleado_sel:
                    nombre = empleado_sel.split(" - ", 1)[1]
                    # MODIFICACIÓN: Se fuerza "Almuerzo" automáticamente
                    ...
                    st.rerun()
                else: st.warning("Selecciona un empleado")
    ...
    with tab_rep1:
        ...
        if st.button("Generar Minuta Institucional", type="primary", use_container_width=True):
            ...
            if not stats: st.error("No hay datos")
                ...
                # Conteos específicos para la nueva minuta
                ...
                cant_lao26 = len([n for n in todas_novedades if "LAO26" in n[2] or n[2] == "LAO"]) # Ajuste según como lo registres
            ...
            if not stats: st.error("No hay datos")
                ...
                for motivo in motivos_unicos:
                    ...
                    for i, item in enumerate(items, 1):
                        if motivo == "ART": minuta += f"{i}. {item[0].upper()} DESDE {item[4]}.\n"
                        else: minuta += f"{i}. {item[0].upper()}.\n"
                    minuta += "\n"
